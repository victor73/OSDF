...
exports.valid_json_schema = function(json_schema) {
    ...
    try {
        ...
        if (json_schema.hasOwnProperty('properties')) {
            valid = checker.validateSchema(json_schema);
        } else {
            logger.debug('Provided schema did not have "properties" field.');
        }
    } catch (err) {
        logger.warn(
            'Unable to determine if JSON-Schema provided was valid.',
            err
        );
    }
...
exports.valid_schema_name = function(schema_name) {
    ...
    if (! result && schema_name.length <= 64) {
