...
var elastic_client = new elasticsearch.Client({ host: elasticsearch_host +
    ':' + elasticsearch_port });
...
elastic_client.indices.existsType(
    {
        index: 'osdf',
        type: 'osdf'
    },
    function(err, result) {
        ...
        if (result === true) {
            console.log('ElasticSearch CouchDB river ' + es_river_name +
                ' exists.');
            process.exit(0);
        } else {
            console.log('ElasticSearch CouchDB river ' + es_river_name +
